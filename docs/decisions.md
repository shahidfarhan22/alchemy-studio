...
## ADR-019 — M8: legal pages, SEO, accessibility baseline

**Context:** M8's goal — Privacy Policy, Terms, Refund/Shipping policy live; product pages indexable; WCAG 2.1 AA baseline met. Confirmed real policy decisions with Zee before drafting anything (not invented): catalog miniatures are non-returnable except for defects/damage (made-to-order, not stock); custom/commissioned orders are non-refundable once accepted except non-delivery or defect (matches ADR-016's "firm commission once paid" model); shipping is domestic-India-only, standard courier, 5–10 business days, with no separate shipping fee (confirmed by reading the actual checkout/order code — there's no `ShippingFee` field anywhere in the system, so the policy states what's actually true rather than promising something unbuilt). Business identity for the pages: proprietor Shahid Farhan, registered address in Chennai, grievance contact `contact@alchemystudios.co.in` (his real working business-domain inbox, confirmed live before use — distinct from the personal Gmail used for Razorpay login and the separate Gmail used only for DMARC aggregate reports, ADR-018).

**Legal pages** (`/privacy`, `/terms`, `/refund-policy`) include the grievance-officer block required by India's Consumer Protection (E-Commerce) Rules, 2020 — named officer, address, email, 48-hour acknowledgment commitment. Built as three new routes reusing the existing Vault UI primitives (`Container`, `PageHeading`, `HairlineRule`) plus two new small shared components: `LegalSection` (heading + prose) and `GrievanceContact` (the repeated officer-contact block, third real use across the three pages). Linked from a new nav in `SiteFooter`.

**SEO**: added `metadataBase` + Open Graph/Twitter defaults to the root layout (pointed at the intended production domain `alchemystudios.co.in` — M9 will confirm/adjust once real hosting is chosen, this only affects how relative URLs in generated meta tags resolve, nothing else). The real gap found by reading the existing code, not assumed: `products/[slug]/page.tsx` had **zero per-page metadata** — every product page rendered with the same generic site title, meaning Google would never distinguish one miniature from another in search results. Added `generateMetadata` there (title/description from the real product, `openGraph.images` from `product.imageUrl` when present), memoized against the page body's own data fetch via React's `cache()` so the backend is only called once per request rather than twice. Added `app/sitemap.ts` (static routes + every real product slug, fetched live) and `app/robots.ts` (disallows admin/cart/checkout/orders/login/register, but explicitly `allow`s `/custom-orders/new` — the one public page under an otherwise-private `/custom-orders/*` prefix — using Next's array `allow`/`disallow` support on a single rule, confirmed against this project's actual installed Next.js version's type definitions before writing it, not assumed from general Next.js knowledge, per this repo's `AGENTS.md` warning that this version may differ from training data).

**Accessibility**: audited what the earlier Vault visual-identity pass (ADR-015) already covered before adding anything — found focus-visible rings, `aria-invalid` wiring (built into the shared `Input`/`Select`/`Textarea` primitives), real alt text on every `<img>`, and `aria-live="polite"` on the order-status page were all already in place. Two real gaps found and fixed: no skip-to-content link (WCAG 2.4.1 Bypass Blocks) — added one at the top of `body`, targeting a `display:contents` wrapper around `{children}` in the root layout so it doesn't disturb any page's own flex layout; and the header's `<nav>` had no `aria-label`, which mattered once the footer gained its own `<nav>` too (now `aria-label="Primary"` vs `aria-label="Legal"`, so screen-reader users can tell the two landmarks apart). Spot-checked color contrast by hand: `--color-muted` (#8b877e) on `--color-bg` (#0a0a0b) computes to ~5.5:1, `--color-gold` on `--color-bg` to ~8.2:1 — both clear the WCAG AA 4.5:1 threshold for normal text.

**Verified:**
- `npm run lint`: clean. `npm run build`: clean, all new routes (`/privacy`, `/terms`, `/refund-policy`, `/robots.txt`, `/sitemap.xml`) compile and appear in the route table.
- Started the real dev server against the real backend and curl'd every new route: legal pages 200, `/robots.txt` shows the correct allow/disallow rules, `/sitemap.xml` correctly includes both static routes and the real product slugs pulled live from the database, a real product page's `<title>` tag confirmed to show the actual product name (not the generic site title).
**What's explicitly deferred:** a full manual screen-reader pass (NVDA/VoiceOver) — this pass fixed the concrete gaps found by reading the code and computing real contrast ratios, not a substitute for Zee clicking through with a screen reader himself.
**Date:** 2026-08-11
